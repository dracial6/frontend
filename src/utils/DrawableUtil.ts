...
class DrawableUtil {
    ...
    static getRectangle(rect: Rectangle, scale: number): Rectangle {
        let returnValue = rect;
        if (scale !== 1) {
            const tx = rect.x * scale;
            const ty = rect.y * scale;
            const twidth = rect.width * scale;
            const theight = rect.height * scale;

            returnValue = new Rectangle(tx, ty, twidth, theight);
        }
