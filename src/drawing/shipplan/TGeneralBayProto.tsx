...
import { Color, FontStyles, FrozenTypes, Point, Rectangle, Size } from "../structures";
...
class TGeneralBayProto extends TBaseGeneral {
        ...
        if (this.VESSEL_DEFINE.vslBays[this.BayIndex].hatchCoverNo > 0) {
            ...
            for (let hatchCoverIndex = 1; hatchCoverIndex <= hatchCoverNO; hatchCoverIndex++) {
                if (this.PROPERTY.isDisplayPiledHatch) {
                    ...
                    if (upHatchCover != null && downHatchCover != null) {
    ...
    protected getHatchCoverStartPos(): Point {
        return new Point(this.padding.left, this.padding.top + this.getTitleBaySize().height + this.getDeckRowNoSize().height + this.getStackWeightSize(false).height + this.getValueBoundaryArea(1).height);
    }
    ...
    protected drawHoldRowNo(): void {
        let startY = this.padding.top + this.getTitleBaySize().height
    ...
    private setHoldRowNoClearBounds(): void {
        const y = this.padding.top + this.getTitleBaySize().height
    ...
    private selectMark(text: GeometryText, isSelect: boolean): void {
        ...
        text.attribute.outLineColor = (isSelect) ? new Color(255, 125, 162, 206) : Color.White();
        text.attribute.fillColor = (isSelect) ? new Color(255, 220, 235, 252) : Color.White();
    ...
    protected getDeckRowNoStartPos(): Point {
        return new Point(this.padding.left, this.padding.top + this.getTitleBaySize().height + (this.PROPERTY.switchRowNoWeight ? this.getDeckStackWeightSize().height : 0))
    }
    ...
    protected getStackWeightTextHeight(defaultHeight: number): number {
        if (this.zoomRatioInfo.zoomRatio !== 1) {
            ...
            if (defaultHeight < stringSize.height) {
                defaultHeight = stringSize.height;
            }
        }
    ...
    protected getTitleBaySize(): Size {
    ...
    protected getRowTextHeight(defaultHeight: number): number {
        ...
        if (this.zoomRatioInfo.zoomRatio !== 1) {
            stringSize = FontUtil.measureText("00", this.zoomRatioInfo.fontName, this.zoomRatioInfo.titleFontSize, this.zoomRatioInfo.titleFontStyle);
    ...
    protected getDeckStackWeightStartPos(): Point {
        return new Point(this.padding.left + this._indicatorGap, this.padding.top + this.getTitleBaySize().height + (this.PROPERTY.switchRowNoWeight ? 0 : this.getDeckStackWeightSize().height));
    }
    ...
    protected getSlotLocationX(startX: number, rowIndex: number, tierIndex: number): number {
        if (ShipUtil.isHold(this.VESSEL_DEFINE, this.BayIndex, tierIndex)) {
            return this.getLeftMarginOfHold() + this.getSlotLocUtil().getSlotPosX(this.VESSEL_DEFINE.vslBays[this.BayIndex].box[rowIndex]) / 4;
        } else {
    ...
    protected getSlotStartPos(): Point {
        const y = this.padding.top + this.getBayTitleSize().height + this.getDeckRowNoSize().height + 1
    ...
    protected getBayTitleSize(): Size {
        return FontUtil.measureText(this.getBayTitle(), this.zoomRatioInfo.fontName, this.zoomRatioInfo.titleFontSize, this.zoomRatioInfo.titleFontStyle);
    }
    ...
    getValueBoundaryArea(holdDeckMode: number): Rectangle {
        ...
        const valueBoundaryList = this.getGeomListByType("displayRowIndex");
        let maxRowIndex = 0;
...
export default TGeneralBayProto;