...
abstract class BaseDrawableObject extends BaseGeometry implements IDrawableMouseEvent {
    ...
    private drawCacheImage(ctx: CanvasRenderingContext2D, drawBoundary: Rectangle, pageScale: number): void {
        if (pageScale !== 1) {
            ...
            ctx.putImageData(this._memoryImage, drawBoundary.x * pageScale + 0.5 + offsetX, drawBoundary.y * pageScale - 0.5 + offsetY);
        } else {
    ...
    getMaxXInMinBoundary(scalef: number): number {
        if (this.degree !== 0) {
            ...
            return (rec.x + rec.width) * scalef + 0.5;
        }

        return (this._currentLocation.x + this._size.width) * scalef + 0.5;
    }

    getMaxYInMinBoundary(scalef: number): number {
        if (this.degree !== 0) {
            const rec = this.getRealRectangle();
            return (rec.y + rec.height) * scalef + 0.5;
        }

        return (this._currentLocation.y + this._size.height) * scalef + 0.5;
    }
    ...
    updateMBR(): void {
        ...
        if (!this._location.equal(Point.Empty())) {
        ...
        if (!tempLocation.equal(Point.Empty())) {
    ...
    private getImage(pageScale: number, ctx: CanvasRenderingContext2D): ImageData {
        ...
        if (!this._memoryImage) {
            ...
            if (this.imagePadding.equal(Padding.Empty)) {
                ...
                imageData = ctx.getImageData(0, 0, rect.width * pageScale + 0.5 + paddingWidth, rect.height * pageScale + 0.5 + paddingHeight);
            }
    ...
    abstract getGeomListByType(memberName: string): IBaseGeometry[];
